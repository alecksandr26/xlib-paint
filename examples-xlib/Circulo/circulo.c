...
int main(int argc,char **argv)
{
  ...
  if ( (display=XOpenDisplay(NULL)) == NULL )
    {
      (void) fprintf( stderr, 
		     "circulo: no puedo conectar al servidor X %s\\n");
      exit( -1 );
    }
  ...
  while (1)  {
    ...
    case Expose:
      ...
      XDrawArc(display, win, gc, currentX-currentR, currentY-currentR,currentR*2,currentR*2,0,360*64);
      XSetFunction(display, gc, GXxor);   
      XSetForeground(display,gc,foreground_pixel);
      break;
    case ButtonPress:
	 ...
	 mode=CHANGERADIO;
         XDrawArc(display, win, gc, currentX-currentR, currentY-currentR,currentR*2,currentR*2,0,360*64);		  
         currentR=distancia(report.xmotion.x,report.xmotion.y,
			    currentX,currentY);
         XDrawArc(display, win, gc, currentX-currentR, currentY-currentR,currentR*2,currentR*2,0,360*64);		  
       }       
       else 
	 mode=NOCHANGE;
      break;
    case MotionNotify:
     if(mode==CHANGECENTER){
        XDrawArc(display, win, gc, currentX-currentR, currentY-currentR,currentR*2,currentR*2,0,360*64);		  
        currentX=report.xmotion.x;
        currentY=report.xmotion.y;
        XDrawArc(display, win, gc, currentX-currentR, currentY-currentR,currentR*2,currentR*2,0,360*64);		  
      }
      else if(mode==CHANGERADIO){
        XDrawArc(display, win, gc, currentX-currentR, currentY-currentR,currentR*2,currentR*2,0,360*64);		  
        currentR=distancia(report.xmotion.x,report.xmotion.y,
          currentX,currentY);
        XDrawArc(display, win, gc, currentX-currentR, currentY-currentR,currentR*2,currentR*2,0,360*64);		  
